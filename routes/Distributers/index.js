...
exp.signup = async (req,res) => {
    ...
    [err , result] = await to(db.query("insert into Distributors(email, number , location, password) values(?,?,?,?)" , [email , number , location , password]));
    if(err)
      return res.sendError(err);
    return res.sendSuccess("Distributer signed up");
};
...
exp.login = async (req,res) =>{
    ...
    [err, userData] = await to(db.query("select * from Distributors where email = ?" ,[email]));
